   ...
   public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            _nums = new List<int>(Counter);
            _tempNumbers = new HashSet<int>(Counter);
            _random = new Random();
        ...
        {
            _nums.AddRange(Enumerable.Range(1, Counter));

            var size = 40;
            for (var i = 0; i < 4; i++)
            for (var j = 0; j < Counter / 4; j++)
            {
                ...
                var number = Randomize(0, Counter);
        ...
        private void ChangeNumber()
        {
            foreach (var item in PanelNumbers?.Controls)
            {
                ...
                {
                    var number = Randomize(_numberNow - 1, Counter);
                    _tempNumbers.Add(number);
                    button.Text = number.ToString();
                }
            }
